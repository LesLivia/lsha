...
RS = config['SUL CONFIGURATION']['RESAMPLE_STRATEGY']
...
class Teacher:
    ...
    def mi_query(self, word: Trace):
        if len(self.flows[0]) > 3: # Valore di threhhold per troppe flow condition
            ...
            if len(segments) > 0:
                ...
                for i, (segment, control) in enumerate(zip_longest(segments, segments_control, fillvalue=None)):
                    ...
                    if use_pysindy == 'True':
                        ...
                        if not np.all(model.coefficients() == 0) and check_model_uniqueness(models, model):
                            models.append(model)
